#!/usr/bin/env bash
# Run the packaged Ananas Qt5 application from the Ubuntu 24.04 build image.
#
# The .deb targets Ubuntu 24.04 (Qt5), so it is installed inside the container
# on the fly.
#
# Usage:
#   run-qt5.sh [application]
#
# Environment:
...
if ! ls "$WORKSPACE_DIR"/dist/ananas_*.deb >/dev/null 2>&1; then
    echo "No package found in $WORKSPACE_DIR/dist. Run build-qt5.sh first." >&2
    exit 1
fi
...
podman run --rm -it \
    ...
    bash -c '
        ...
        # --force-depends: libqdataschema is provided by the image but not in dpkg
