#!/usr/bin/env bash
# Build the libqdataschema Debian package for Qt6 in the ananas-qt6-builder
# image (Ubuntu 24.04), the same way the main ananas package is built.
#
# Usage: build-qt6-qdataschema.sh
#
# Environment:
#   QDS_BRANCH    qdataschema branch to build (default: qt6)
#   ANANAS_IMAGE  build image to use (default: ananas-qt6-builder)
set -e

WORKSPACE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
REPO="$WORKSPACE_DIR/ananas-legacy-qdataschema"
BRANCH="${QDS_BRANCH:-qt6}"
...
echo "===> 1. Сборка Podman-образа (Ubuntu 24.04 + Qt6)..."
podman build -t "$IMAGE" -f "$WORKSPACE_DIR/tools/docker/Containerfile.qt6" "$WORKSPACE_DIR"
