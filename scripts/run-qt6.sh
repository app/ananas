#!/usr/bin/env bash
# Run the packaged Ananas Qt6 application from the Ubuntu 24.04 build image.
#
# Usage:
#   run-qt6.sh [application]
#
# Environment:
...
if ! ls "$WORKSPACE_DIR"/dist/ananas_*.deb >/dev/null 2>&1; then
    echo "No package found in $WORKSPACE_DIR/dist. Run build-qt6.sh first." >&2
    exit 1
fi
